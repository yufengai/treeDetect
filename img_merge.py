...
def meger_main(conf):
    ...
    for root, dirs, files in os.walk(conf.get('tiles_path')):
        if a == 0:
            img_dir_num = len(dirs)
            start_y = int(dirs[0])
        elif a == 1:
            ...
            break
        a += 1
    pingjie_multilple(conf.get('tiles_path'), img_number, img_dir_num, conf.get('x_tiles_nums'), conf.get('y_tiles_nums')
                      , start_x, start_y, conf.get('x_tile_size'), conf.get('y_tile_size'), matrix)
